...
# функция которая помещает рандомно 2 или 4 в map
def place_rand_2(m):
    l = zero_place(m)
    if len(l) == 0:
        return
    a = random.choice(l)
    m[a[0]][a[1]] = 4 if random.random() < 0.1 else 2 
...
def move_marge(m):
    ...
    for row in m:
        ...
        # делаю слияние и меняю второе число на 0
        for i in range(len(new_row) - 1):
            if new_row[i] == new_row[i + 1]:
                new_row[i] *= 2
                new_row[i + 1] = 0
                reward += math.log2(new_row[i]) / 4  
        # убираю нули
...
#     mid = [(1, 1), (1, 2), (2, 1), (2, 2)]
#     is_in_mid = any(map[i][j] == max_val for i, j in mid)
#     if is_in_mid:
#         reward += -3
...
#             if (map[0][i] == (map[0][i - 1] / 2)) and ((map[0][i] != 0) and (map[0][-1] != 0)):
...
def monotonicity(map):
    reward = 0
    if np.array(map).max() != map[0][0]:
        return reward
    #даю награду если в левом верхнем углу максимальное
    reward += np.log2(map[0][0])
    #я прохожусь по первой строчке, чтобы посмотреть монотонность
    for i in range(3):
        if map[0][i] == map[0][i+1]*2:
            reward+= np.log2(map[0][i+1])
        else:
            return reward
    #прохожусь по второй строчке начиная справа налево
    if map[0][3] == map[1][3]*2:
        reward+= np.log2(map[1][3])
    else:
        return reward
    for i in range(3, 0, -1):
        if map[1][i] == map[1][i-1]*2:
            reward+= np.log2(map[1][i-1])
        else:
            return reward
    #прохожусь по третьей строчке начиная слева направо
    if map[1][0] == map[2][0]*2:
        reward+= np.log2(map[2][0])
    else:
        return reward
    for i in range(3):
        if map[2][i] == map[2][i+1]*2:
            reward+= np.log2(map[2][i+1])
        else:
            return reward
    #прохожусь по четвертой строчке
    if map[2][0] == map[3][0]*2:
        reward+= np.log2(map[3][3])
    else:
        return reward
    for i in range(3, 0, -1):
        if map[3][i] == map[3][i-1]*2:
            reward+= np.log2(map[3][i-1])
        else:
            return reward       
